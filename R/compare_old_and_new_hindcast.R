...
# read files from loon
# probably will need to update the path when the folder structure on GitHub is in place
# 1000 m
old_hind_1000 <- read.csv('Data/output_from_loon/nep_avg_hind.csv', header = T)
new_hind_1000 <- read.csv('Data/output_from_loon/nep_avg_revised_hind_1000.csv', header = T)

# 300 m
old_hind_300 <- read.csv('Data/output_from_loon/nep_avg_hind_300.csv', header = T)
new_hind_300 <- read.csv('Data/output_from_loon/nep_avg_revised_hind_300.csv', header = T)

# start from 1000 m
old_hind_1000 <- old_hind_1000 %>%
  mutate(run = 'old', year = year(date), month = month(date))

new_hind_1000 <- new_hind_1000 %>%
  mutate(run = 'new', year = year(date), month = month(date))

hind_1000 <- rbind(old_hind_1000, new_hind_1000) %>%
  filter(year >= 2000)
